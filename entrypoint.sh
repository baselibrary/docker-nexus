...
if [ "$1" == '/opt/sonatype/nexus/bin/nexus' ]; then
  if [ ! -f "$NEXUS_CERT/keystore.jks" ]; then
    ...
    if [ ! -f $NEXUS_CERT/nexus.key ]; then
      $JAVA_HOME/bin/keytool -genkeypair -alias nexus -dname "CN=$NEXUS_CERT_CN" -ext "SAN=dns:nexus.thoughtworks.io" -keyalg RSA -keysize 2048 -keystore $NEXUS_CERT/nexus.key -keypass $NEXUS_CERT_PASSWORD -storepass $NEXUS_CERT_PASSWORD
    fi
