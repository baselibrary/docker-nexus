...
if [ "$1" == '/opt/sonatype/nexus/bin/nexus' ]; then
  if [ ! -f "$NEXUS_CERT/keystore.jks" ]; then
    ...
    $JAVA_HOME/bin/keytool -importkeystore -noprompt -srckeystore $NEXUS_CERT/nexus.key -srcstorepass $NEXUS_CERT_PASSWORD -srcstoretype pkcs12 -destkeystore $NEXUS_CERT/keystore.jks -deststorepass $NEXUS_CERT_PASSWORD -deststoretype pkcs12
